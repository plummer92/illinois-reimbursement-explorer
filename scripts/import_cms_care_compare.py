...
def quality_record(row: dict[str, str]) -> dict[str, object]:
    return {
        ...
        "legalBusinessName": row.get("Legal Business Name", "").strip(),
        ...
        "averageResidentsPerDay": number_or_none(row.get("Average Number of Residents per Day")),
        "providerType": row.get("Provider Type", "").strip(),
        "providerResidesInHospital": row.get("Provider Resides in Hospital", "").strip(),
        "dateFirstApproved": row.get("Date First Approved to Provide Medicare and Medicaid Services", "").strip(),
        "chainName": row.get("Chain Name", "").strip(),
        "chainId": row.get("Chain ID", "").strip(),
        "numberOfFacilitiesInChain": int_or_none(row.get("Number of Facilities in Chain")),
        "chainAverageOverallRating": number_or_none(row.get("Chain Average Overall 5-star Rating")),
        "chainAverageHealthInspectionRating": number_or_none(row.get("Chain Average Health Inspection Rating")),
        "chainAverageStaffingRating": number_or_none(row.get("Chain Average Staffing Rating")),
        "chainAverageQualityMeasureRating": number_or_none(row.get("Chain Average QM Rating")),
        "continuingCareRetirementCommunity": row.get("Continuing Care Retirement Community", "").strip(),
        "specialFocusStatus": row.get("Special Focus Status", "").strip(),
        "abuseIcon": row.get("Abuse Icon", "").strip(),
        "providerChangedOwnershipLast12Months": row.get("Provider Changed Ownership in Last 12 Months", "").strip(),
        "residentFamilyCouncil": row.get("With a Resident and Family Council", "").strip(),
        "automaticSprinklers": row.get("Automatic Sprinkler Systems in All Required Areas", "").strip(),
        ...
        "nurseAideHoursPerResidentDay": number_or_none(row.get("Reported Nurse Aide Staffing Hours per Resident per Day")),
        "lpnHoursPerResidentDay": number_or_none(row.get("Reported LPN Staffing Hours per Resident per Day")),
        "rnStaffingHoursPerResidentDay": number_or_none(row.get("Reported RN Staffing Hours per Resident per Day")),
        "licensedStaffingHoursPerResidentDay": number_or_none(row.get("Reported Licensed Staffing Hours per Resident per Day")),
        "totalNurseStaffingHoursPerResidentDay": number_or_none(row.get("Reported Total Nurse Staffing Hours per Resident per Day")),
        "weekendTotalNurseStaffingHoursPerResidentDay": number_or_none(row.get("Total number of nurse staff hours per resident per day on the weekend")),
        "weekendRnHoursPerResidentDay": number_or_none(row.get("Registered Nurse hours per resident per day on the weekend")),
        "physicalTherapistHoursPerResidentDay": number_or_none(row.get("Reported Physical Therapist Staffing Hours per Resident Per Day")),
        "totalNursingStaffTurnover": number_or_none(row.get("Total nursing staff turnover")),
        "registeredNurseTurnover": number_or_none(row.get("Registered Nurse turnover")),
        "administratorTurnoverCount": int_or_none(row.get("Number of administrators who have left the nursing home")),
        "nursingCaseMixIndex": number_or_none(row.get("Nursing Case-Mix Index")),
        "adjustedRnHoursPerResidentDay": number_or_none(row.get("Adjusted RN Staffing Hours per Resident per Day")),
        "adjustedTotalNurseStaffingHoursPerResidentDay": number_or_none(row.get("Adjusted Total Nurse Staffing Hours per Resident per Day")),
        "ratingCycle1HealthDeficiencies": int_or_none(row.get("Rating Cycle 1 Total Number of Health Deficiencies")),
        "ratingCycle1TotalHealthScore": number_or_none(row.get("Rating Cycle 1 Total Health Score")),
        "totalWeightedHealthSurveyScore": number_or_none(row.get("Total Weighted Health Survey Score")),
        "infectionControlCitations": int_or_none(row.get("Number of Citations from Infection Control Inspections")),
        "numberOfFines": int_or_none(row.get("Number of Fines")),
        "totalFinesDollars": number_or_none(row.get("Total Amount of Fines in Dollars")),
        "numberOfPaymentDenials": int_or_none(row.get("Number of Payment Denials")),
        "totalNumberOfPenalties": int_or_none(row.get("Total Number of Penalties")),
